...
package upstream does not have, `plugins/browser-plugin-engagement-time`. The fork does not carry
...
| `trackers/javascript-tracker/src/features.ts` | wires `browser-plugin-engagement-time`, activated whenever the bundle carries it so the `enableEngagementTime` command exists for GTM containers; `tracker.lite.config.ts` and `tracker.test.config.ts` set `engagementTime = true`, `tracker.config.ts` (sp.js) sets it false |
| `trackers/javascript-tracker/src/configuration.ts` | the `contexts.engagementTime` option (`boolean` or `{ piggyback }`) |
| `plugins/browser-plugin-engagement-time/` | the whole package: GA4-rule engagement time, scroll depth and interaction counts per page view, reported as `iglu:to.flip/ft_engagement_time/jsonschema/1-0-0`, whose schema file lives in its `schemas/`. Inert until a tracker enables it; see its README |
